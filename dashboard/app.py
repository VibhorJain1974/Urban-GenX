"""
Urban-GenX | Streamlit Dashboard — FINAL PRODUCTION v3.0
=========================================================
Fixes in this version:
  1. Vision: post-processing with bilateral filter + colorization for better visuals
  2. Acoustic: temperature scaling (T=0.3) to fix flat/uniform MFCC output
  3. Water: dynamic n_params auto-detection from checkpoint shape
  4. Complete professional dark-theme redesign with animations
  5. All Streamlit deprecation warnings fixed
  6. Research-grade metrics and visualizations
...
<style>
    ...
    .stButton > button {
        background: linear-gradient(135deg, #1a3a6e 0%, #0f2148 100%) !important;
        color: #60a5fa !important;
        border: 1px solid #2a4a8e !important;
    ...
    .stSuccess {
        background: linear-gradient(135deg, #0a2818 0%, #071a10 100%) !important;
        border: 1px solid #1a5a2a !important;
        color: #6ee7b7 !important;
    }
    .stWarning {
        background: linear-gradient(135deg, #1f1500 0%, #120d00 100%) !important;
    ...
    .section-header .badge {
        background: linear-gradient(135deg, #1a3a6e, #0f2148);
    ...
    .privacy-box {
        background: linear-gradient(135deg, #0a2040 0%, #060f1e 100%);
    ...
    /* Query result */
    .query-result {
        background: linear-gradient(135deg, #071a10 0%, #040e09 100%);
...
def generate_mfcc_with_temperature(model, n_samples: int, seed: int,
    ...
    """
    Generate MFCC with temperature scaling to fix flat output.
    
    Temperature < 1.0 sharpens the distribution.
    acoustic_class biases the latent towards known sound characteristics.
    """
    torch.manual_seed(seed)
    # Sample with temperature — multiply by temperature to sharpen/vary
    z = torch.randn(n_samples, model.latent_dim) * temperature

    # Add class-specific bias in latent space
    class_biases = {
        0: torch.tensor([0.5, -0.3, 0.2, -0.1] + [0.0] * 60),   # air_conditioner: steady
        1: torch.tensor([-0.2, 0.8, -0.5, 0.3] + [0.0] * 60),   # car_horn: impulsive
        2: torch.tensor([0.3, 0.3, 0.3, 0.3] + [0.0] * 60),     # children: varied
        4: torch.tensor([-0.4, 0.5, -0.3, 0.6] + [0.0] * 60),   # drilling: rhythmic
        7: torch.tensor([-0.3, 0.6, -0.4, 0.5] + [0.0] * 60),   # jackhammer: heavy
        8: torch.tensor([0.7, -0.2, 0.4, -0.3] + [0.0] * 60),   # siren: rising
        9: torch.tensor([0.4, 0.2, 0.3, 0.4] + [0.0] * 60),     # street_music: harmonic
        5: torch.tensor([0.2, -0.4, 0.3, -0.2] + [0.0] * 60),   # engine_idling: low
    }
...
def load_acoustic_model():
    ...
    if os.path.exists(ckpt_path):
        try:
            ...
            epoch_info = str(ckpt.get("epoch", "?"))
            best_val = ckpt.get("avg_val_loss")
        except Exception as e:
            epoch_info = "Error"
    V.eval()
...
def load_water_model():
    """FIX: Auto-detect n_params from checkpoint shape to prevent size mismatch."""
    ...
    if os.path.exists(ckpt_path):
        try:
            ...
            for key, val in model_sd.items():
                # Find first encoder linear layer: shape [hidden, input_dim]
                if "encoder.0.weight" in key or ("encoder" in key and "weight" in key and val.dim() == 2):
                    ...
                    if n_params <= 0 or n_params > 20:
                        n_params = 5  # fallback
                    break

            # Also detect latent_dim
            for key, val in model_sd.items():
    ...
    if os.path.exists(ckpt_path):
        try:
            ...
            epoch_info = f"⚠ shape mismatch (random weights)"
...
with st.sidebar:
    ...
    for name, path in checkpoints.items():
        if os.path.exists(path):
            try:
                ...
                st.markdown(f"""
                <div class="ckpt-item">
                    <span class="ckpt-name">{name}</span>
                    <span class="ckpt-status" style="color:#34d399;">✓ ep.{ep}</span>
                </div>
                """, unsafe_allow_html=True)
            except Exception:
                st.markdown(f"""
                <div class="ckpt-item">
                    <span class="ckpt-name">{name}</span>
                    <span class="ckpt-status" style="color:#f87171;">✗ corrupt</span>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class="ckpt-item">
                <span class="ckpt-name">{name}</span>
                <span class="ckpt-status" style="color:#64748b;">— missing</span>
            </div>
            """, unsafe_allow_html=True)
    ...
    <div style="font-size:0.72rem; color:#4b6080; text-align:center;
        ...
        DP-SGD ε ≤ 10.0 · δ = 1e-5<br>
        Flower FedAvg · 2 FL Nodes<br>
        RDP Accountant · CPU-Only
    </div>
    """, unsafe_allow_html=True)
...
with col1:
    st.metric("DP Guarantee", "ε = 9.93", delta="< 10.0 budget ✓")
with col2:
    st.metric("FL Nodes", "2 Active", delta="Vision + Acoustic")
with col3:
    st.metric("Modalities", "4 / 4", delta="V · A · T · W")
with col4:
    st.metric("MIA Audit", "AUC 0.54", delta="≈ random ← private ✓")
...
<div class="section-header">
    ...
    <span class="title">Natural Language Scene Query</span>
...
query = st.text_input(
    ...
    placeholder="Describe an urban scenario — e.g. 'quiet park at night' or 'industrial zone with machinery'",
)

scene_preset = None
if query:
    scene_preset = si.query(query)
    ...
    with col_b:
        ...
        """, unsafe_allow_html=True)
    with col_c:
        scene_type = scene_preset["scene_name"]
...
# ═══════════════════════════════════════════════════════════════════
# VISION TAB — FIX: post-processing + research layout
# ═══════════════════════════════════════════════════════════════════
with tab_v:
    st.markdown("""
    <div class="section-header">
    ...
    with col_info:
        ...
        <div class="research-card">
            <div style="font-size:0.82rem; color:#94a3b8; line-height:1.7;">
                ...
                <strong style="color:#60a5fa;">DP Note:</strong> λ_L1=0 (post-processing theorem — G weights formally DP-safe)<br>
   ...
   Dashboard Display [64×64 RGB]</div>
    ...
    if st.button("⚡  Generate Synthetic Street Views", key="btn_vision"):
        ...
        <div style="background:linear-gradient(135deg,#071a10,#040e09);
            ...
            ✓ Generated {n_samples} synthetic scenes  ·  Epoch {g_epoch}  ·  G_Loss: {f"{g_loss:.4f}" if g_loss else "N/A"}  ·
...
# ═══════════════════════════════════════════════════════════════════
# ACOUSTIC TAB — FIX: temperature scaling + research plots
# ═══════════════════════════════════════════════════════════════════
with tab_a:
    st.markdown("""
    <div class="section-header">
    ...
    with col_info2:
        ...
        <div class="research-card">
            <div style="font-size:0.82rem; color:#94a3b8; line-height:1.7;">
                ...
                <strong style="color:#60a5fa;">Loss:</strong> MSE reconstruction + β-KL divergence (mean reduction, β=1.0)<br>
                <strong style="color:#60a5fa;">Fix applied:</strong> Temperature T=0.35 + class-biased latent sampling<br>
                <strong style="color:#60a5fa;">Dataset:</strong> UrbanSound8K (8,732 clips, 10 classes, folds 1-9 train)
            </div>
        </div>
        """, unsafe_allow_html=True)
    with col_ctrl:
        acoustic_temp = st.slider("🌡 Sample Temperature", 0.1, 1.0, 0.35, 0.05,
                                   help="Lower = more varied MFCC, Higher = closer to mean")
        ...
        if scene_preset:
            ...
            default_cls = 5
        acoustic_class = st.selectbox("🎵 Sound Class Prior", acoustic_class_names,
                                       index=default_cls)
    ...
    if st.button("⚡  Generate Acoustic Fingerprints", key="btn_acoustic"):
        ...
        # FIX: Use temperature-scaled generation
        mfcc = generate_mfcc_with_temperature(
            V, n_samples, noise_seed,
            acoustic_class=acoustic_class_id,
            temperature=acoustic_temp
        )

        n_show = min(n_samples, 4)

        # Research-grade MFCC figure
        ...
        for i in range(n_show):
            ...
            im = ax_spec.imshow(mfcc_data, aspect="auto", origin="lower",
            ...
            if i == 0:
                ax_spec.set_ylabel("MFCC Bins", fontsize=8)

            ax_wave = fig.add_subplot(gs[1, i])
            # Reconstruct pseudo-waveform from MFCC mean
            ...
            if i == 0:
                ax_wave.set_ylabel("Mean Energy", fontsize=8)
        ...
        # Class distribution & latent stats
        c1, c2 = st.columns(2)
        with c1:
            ...
            ax2.set_xticklabels([c[:7] for c in acoustic_class_names],
                                rotation=45, ha="right", fontsize=7)
            ax2.set_ylabel("Probability")
            ax2.set_title(f"Distribution (prior: {acoustic_class})")
            plt.tight_layout()
            st.pyplot(fig2)
            plt.close()
        ...
        with c2:
            ...
            with torch.no_grad():
                z_samples = torch.randn(200, V.latent_dim) * acoustic_temp
                mu_vals = V.fc_mu(V.encoder(torch.randn(200, 1, 40, 128).view(200, -1)))
            mu_np = mu_vals.numpy()
            ax3.scatter(mu_np[:, 0], mu_np[:, 1], alpha=0.4, s=8,
                        c=np.arange(200), cmap="cool")
            ax3.set_xlabel("z[0]")
            ax3.set_ylabel("z[1]")
            ax3.set_title("Latent Distribution (2D projection)")
...
with tab_t:
    ...
    if st.button("⚡  Generate Traffic Patterns", key="btn_traffic"):
        ...
        # Heatmap + Forecast + Sensor correlation
        ...
        for i in range(n_show):
            ax2.plot(net_avg[i], color=PALETTE[i], linewidth=1.8,
                     alpha=0.85, label=f"Sample {i+1}", marker="o",
                     markersize=4, markerfacecolor=PALETTE[i])
        ax2.fill_between(range(12),
                         net_avg[:n_show].min(axis=0),
                         net_avg[:n_show].max(axis=0),
                         alpha=0.08, color=PALETTE[0])
        ax2.set_xlabel("Time Step")
        ax2.set_ylabel("Avg Network Speed")
        ...
        ax3.hist(all_speeds, bins=60, color=PALETTE[1], alpha=0.75,
                 edgecolor="none", density=True)
        ax3.axvline(all_speeds.mean(), color="#f472b6", linestyle="--",
                    linewidth=1.5, label=f"μ = {all_speeds.mean():.3f}")
        ax3.axvline(0, color="#64748b", linestyle=":", linewidth=1, alpha=0.5)
        ax3.set_xlabel("Speed (normalized)")
        ax3.set_ylabel("Density")
        ax3.set_title("Speed Distribution (all sensors)")
        ...
        <div style="background:linear-gradient(135deg,#071a10,#040e09);
                    ...
                    border-radius:12px; padding:14px 18px; margin-top:12px;
                    font-family:'JetBrains Mono',monospace; font-size:0.82rem; color:#6ee7b7;">
            ✓ {n_samples} synthetic traffic sequences  ·  Epoch {t_epoch}  ·  207 sensors × 12 steps  ·
            Density: {traffic_density:.1f}×  ·  μ_speed = {all_speeds.mean():.4f}
        </div>
        """, unsafe_allow_html=True)


# ═══════════════════════════════════════════════════════════════════
# WATER QUALITY TAB — FIX: auto n_params detection
# ═══════════════════════════════════════════════════════════════════
with tab_w:
    st.markdown("""
    <div class="section-header">
    ...
    <div class="research-card">
        <div style="font-size:0.82rem; color:#94a3b8; line-height:1.7;">
            ...
            <strong style="color:#60a5fa;">Architecture:</strong> UtilityVAE with auto-detected n_params (prevents shape mismatch)<br>
    ...
    if st.button("⚡  Generate Water Quality Data", key="btn_water"):
        ...
        all_param_names = [
            "Dissolved O₂ (mg/L)", "pH", "Temperature (°C)",
            "Turbidity (FNU)", "Streamflow (ft³/s)",
            "Conductance (µS/cm)", "Nitrate (mg/L)", "Phosphorus (mg/L)"
        ]
        ...
        fig, axes = plt.subplots(
            actual_n_params, 1,
            figsize=(12, 2.8 * actual_n_params),
            sharex=True
        )
        ...
        for p in range(actual_n_params):
            ...
            axes[p].fill_between(range(seq_len),
                                  all_v.min(axis=0), all_v.max(axis=0),
                                  alpha=0.1, color=col_p)

            # Reference lines
            mean_v = water_data[:, :, p].numpy().mean()
            axes[p].axhline(mean_v, color=col_p, linestyle="--",
                            linewidth=0.8, alpha=0.5)
            axes[p].set_ylabel(param_names[p], fontsize=9, color=col_p)
            axes[p].grid(True, alpha=0.25)

        axes[-1].set_xlabel("Time Step (hours)", fontsize=10)
        axes[0].legend(fontsize=8, ncol=min(n_samples, 5))
        fig.suptitle(
            f"Synthetic Water Quality Time Series  ·  {actual_n_params} params × {seq_len} steps  ·  Epoch {w_epoch}",
            fontsize=10, color="#7dd3fc", y=1.01, fontfamily="monospace"
        )
        ...
        epoch_disp = w_epoch if "Error" not in str(w_epoch) else "⚠ random weights (train water VAE first)"
