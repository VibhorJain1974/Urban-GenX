"""
Urban-GenX | Vision Node Training  (FINAL FIX - all bugs resolved)
===================================================================
Fixes:
  1. Opacus 'multiple values for accountant' - rdp set ONLY on PrivacyEngine()
  ...
  6. 'Per sample gradient not initialized' after NaN skip - skip opt_d.step() too
...
def train():
    ...
    if CFG["dp_enabled"]:
        ...
        print(f"[DP] Attached | e={CFG['target_epsilon']} d={CFG['target_delta']}"
              f" remaining_epochs={remaining_epochs}")

    # ── Load checkpoint weights AFTER DP wrapping ──────────────────
    if ckpt is not None:
    ...
    for epoch in range(start_epoch, CFG["num_epochs"]):
        G.train(); D.train()
        epoch_d = 0.0; epoch_g = 0.0; seen = 0; consecutive_nan = 0

        pbar = tqdm(loader, desc=f"Epoch [{epoch+1}/{CFG['num_epochs']}]", unit="batch")
        for batch_idx, (real_img, cond) in enumerate(pbar):
            # Opacus Poisson sampling can emit empty batches; skip them before
            # touching the DP optimizer so per-sample gradients stay valid.
            ...
            d_loss = 0.5 * (criterion(real_pred, real_labels) + criterion(fake_pred, fake_labels))

            if not torch.isfinite(d_loss):
                consecutive_nan += 1
                if consecutive_nan >= CFG["max_consecutive_nan"]:
                    raise RuntimeError(f"{consecutive_nan} consecutive NaN batches. Training unstable.")
                pbar.set_postfix(D_Loss="NaN(skip)", G_Loss="---")
                # CRITICAL: skip the optimizer step entirely.
                # Opacus requires backward before stepping.
                # No backward ran, so no step allowed.
            ...
            g_loss_gan = criterion(fake_pred, torch.full_like(fake_pred, CFG["real_label"]))
        ...
        if is_finite(avg_d) and is_finite(avg_g):
            ...
            print(f"[WARN] Epoch {epoch+1} has non-finite loss. Checkpoint NOT saved.")
