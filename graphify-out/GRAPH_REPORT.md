...
## Summary
- 296 nodes · 417 edges · 30 communities detected
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 93 edges (avg confidence: 0.8)
...
2. `SemanticInterface` - 12 edges
3. `UtilityVAE` - 12 edges
4. `Generator` - 12 edges
5. `train()` - 12 edges
6. `loss()` - 11 edges
7. `train()` - 11 edges
8. `train()` - 11 edges
...
- `test_vision_generator_shape()` --calls--> `Generator`  [INFERRED]
  tests\test_models.py → models\vision_gan.py
...
Cohesion: 0.09
Nodes (34): notify(), notify_crash_save(), notify_epoch(), notify_error(), notify_training_complete(), Urban-GenX | Remote Monitoring Sends push notifications to ntfy.sh topic: vibho, Send push notification to mobile via ntfy.sh.     Priority: min / low / default, get_beta() (+26 more)

### Community 1 - "Dashboard & UI Layer"
Cohesion: 0.08
Nodes (28): enhance_vision_output(), generate_mfcc_with_temperature(), load_acoustic_model(), load_semantic_interface(), load_traffic_model(), load_vision_model(), load_water_model(), Urban-GenX | Streamlit Dashboard — FINAL PRODUCTION v3.0 ======================= (+20 more)
...
Cohesion: 0.14
Nodes (12): check_semantic_interface(), test_semantic_interface(), test_semantic_list_scenes(), list_scenes(), Urban-GenX | Semantic Interface (Cross-Modal Transformer) Maps natural language, Maps natural language queries to urban scene presets.     Primary: sentence-tra, Pre-compute embeddings for all anchor phrases of all presets., Return best-matching preset name using cosine similarity. (+4 more)

### Community 5 - "Vision Generator"
Cohesion: 0.15
Nodes (11): check_vision_gan(), test_vision_discriminator_shape(), Urban-GenX | Vision Node Training — PRODUCTION FINAL ===========================, set_requires_grad(), train(), validate_checkpoint(), Discriminator, Generator (+3 more)
...
Cohesion: 0.17
Nodes (9): AcousticVAE, generate(), interpolate(), Urban-GenX | Acoustic Node VAE over MFCC spectrograms from UrbanSound8K.  Inp, Reparameterization trick: z = mu + eps * sigma  (differentiable sampling)., x: [B, 1, mfcc_bins, time_frames] → (mu, log_var) each [B, latent_dim], z: [B, latent_dim] → [B, 1, mfcc_bins, time_frames], Full forward pass. Returns (reconstruction, mu, log_var). (+1 more)

### Community 9 - "Privacy Auditing"
Cohesion: 0.29
Nodes (9): membership_inference_attack(), Urban-GenX | Privacy Audit Module (FINAL — Standalone Runnable) ================, Run MIA on the trained AcousticVAE., Run MIA on the trained Traffic UtilityVAE., Run MIA on all available models and print summary., Confidence-based Membership Inference Attack (MIA).      For VAE models: uses re, run_acoustic_mia(), run_all_mia() (+1 more)
...
Cohesion: 0.22
Nodes (3): tests/check_all.py ================== Urban-GenX end-to-end smoke-test suite (9, Decorator: wraps a check function, catches exceptions, records result., run_check()
...
## Knowledge Gaps
- **99 isolated node(s):** `fl_debug_plan.py ================ Urban-GenX — Federated Learning Debug & Fix Pl`, `Urban-GenX | Streamlit Dashboard — FINAL PRODUCTION v3.0 =======================`, `Post-process cGAN output to improve visual quality.          The raw DP-GAN outp`, `Generate MFCC with temperature scaling to fix flat output.          Temperature`, `FIX: Auto-detect n_params from checkpoint shape to prevent size mismatch.` (+94 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Server Utilities`** (2 nodes): `main()`, `run_graphify.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Train Vision`** (1 nodes): `Beta-VAE ELBO loss.          FIX: reduction='mean' (not 'sum').           - '`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Train Acoustic`** (1 nodes): `Sample from N(0,I) latent space → synthesize MFCC spectrogram.         Returns:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Metrics & Evaluation`** (1 nodes): `Linearly interpolate in latent space between two MFCC inputs.         Useful fo`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Dataset References`** (1 nodes): `Sample from latent space → synthesize utility vector`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Dependencies`** (1 nodes): `Latent space interpolation between two utility states (counterfactual)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Architecture Docs`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Model Components`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Infrastructure Config`** (1 nodes): `USGS Water Quality Dataset`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `train()` connect `Vision Generator` to `Utility Training Pipeline`, `Data Loading & Preprocessing`, `Acoustic FL Client`, `Vision FL Client`?**
  _High betweenness centrality (0.123) - this node is a cross-community bridge._
- **Why does `AcousticVAE` connect `Acoustic VAE` to `Utility Training Pipeline`, `Dashboard & UI Layer`, `Acoustic FL Client`, `Privacy Auditing`?**
  _High betweenness centrality (0.113) - this node is a cross-community bridge._
- **Why does `train()` connect `Utility Training Pipeline` to `Acoustic VAE`, `Data Loading & Preprocessing`?**
...
- **Are the 6 inferred relationships involving `Generator` (e.g. with `load_vision_model()` and `.__init__()`) actually correct?**
