"""
Urban-GenX | Streamlit Dashboard (PRODUCTION — all bugs fixed)
===============================================================
Fixes:
  1. All Streamlit deprecation warnings resolved
  2. Water VAE: dynamically detects n_params from checkpoint (no hardcoded 5)
  3. Graceful error handling when checkpoint shape mismatches
  4. Uses `use_container_width` → `width="stretch"` everywhere
...
def load_water_model():
    """
    PRODUCTION FIX: Dynamically detect n_params from checkpoint.
    Previous code hardcoded n_params=5, but training may produce 2 or 3 params
    depending on USGS data availability. This caused shape mismatch errors.
    """
    ...
    if os.path.exists(ckpt_path):
        try:
            ...
            # Auto-detect input_dim from checkpoint to determine n_params
            # The first encoder layer weight shape is [hidden_dim, input_dim]
            model_sd = ckpt["model"]
    ...
    W = build_water_vae(seq_len=seq_len, n_params=n_params, latent_dim=min(16, max(8, n_params * 2)))
...
if query:
    ...
    with col_a:
        st.success(
            f"**Matched Scene:** {preset['scene_name'].replace('_', ' ').title()}"
        )
    ...
    if st.button("🎲 Generate Synthetic Street Views", key="btn_vision"):
        ...
        if query:
            ...
            cond = si.build_condition_tensor(
                preset, img_size=64, num_classes=35, batch_size=n_samples
            )
        else:
...
with tab_acoustic:
    ...
    if st.button("🎲 Generate Acoustic Fingerprints", key="btn_acoustic"):
        ...
        fig, axes = plt.subplots(
            1, min(n_samples, 4), figsize=(4 * min(n_samples, 4), 3)
        )
...
with tab_water:
    ...
    if st.button("🎲 Generate Water Quality Data", key="btn_water"):
        ...
        # Parameter names: use as many as we actually have
        all_param_names = [
            "Dissolved O₂", "pH", "Temperature", "Turbidity", "Streamflow",
        ...
        fig, axes = plt.subplots(
            actual_n_params, 1,
            figsize=(10, 2.5 * actual_n_params),
            sharex=True,
        )
        ...
        plt.suptitle(
            "Synthetic Water Quality Time Series", fontsize=13, fontweight="bold"
        )
        plt.tight_layout()
        st.pyplot(fig)
        plt.close()
        ...
        st.info(
            ...
            f"Error: Error(s) in loading state_dict for UtilityVAE: size misma..."
            if w_epoch and "Error" in str(w_epoch)
            else f"✅ Synthetic water quality data | Model epoch: {w_epoch} | "
...
with tab_sdg:
    ...
    st.markdown(
        "Policy impact indicators based on counterfactual slider settings:"
    )
    ...
    ax.set_ylim(0, 100)
    ax.set_title(
        "SDG 11 Compliance Radar", fontsize=12, fontweight="bold", pad=20
    )
    st.pyplot(fig)
    plt.close()
